...
Please create file: $allowed_cameras_file in this format based on BaseRepo4 camera_msgs.py CameraEnum\'s
{
\"allowed_cameras\": [
        \"wrist_camera_left\",
        \"wrist_camera_right\",
        \"handset_wrist_camera\"
    ]
}"
    exit 1
fi
...
for camera in $(jq -r '.cameras | keys[]' "$json_file"); do
    ...
    launch_command="$ros2s; ros2 launch realsense2_camera rs_launch.py serial_no:=_$serial camera_name:=$camera rgb_camera.color_profile:=${width}x${height}x30 depth_module.depth_profile:=${width}x${height}x30 depth_module.color_profile:=${width}x${height}x30 align_depth.enable:=true hole_filling_filter.enable:=true enable_shm:=true"
    # launch_command="$ros2s; ros2 launch realsense2_camera rs_launch.py serial_no:=_$serial camera_name:=$camera rgb_camera.color_profile:=${width}x${height}x30 depth_module.depth_profile:=${width}x${height}x30 depth_module.color_profile:=${width}x${height}x30 align_depth.enable:=true hole_filling_filter.enable:=true pointcloud.enable:=true pointcloud.ordered_pc:=true enable_shm:=true"
    echo "$launch_command"
